...
p=json.load(open(root/"general-round27-root-draft.json"))
p={k:v for k,v in p.items() if v.get("website") and (not (cache/f"{k}.json").exists() or json.load(open(cache/f"{k}.json")).get("requested_url")!=v["website"])}
with concurrent.futures.ThreadPoolExecutor(max_workers=8) as ex:
 for k,v in ex.map(one,p.items()): print(k,v.get("http_status"),flush=True)
