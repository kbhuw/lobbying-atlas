...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round76-input.json'));exec((r/'check_round76_incremental.py').read_text().split('p=json.load(open(root/')[0]);dest=r/'round76-image-audit-middle';dest.mkdir(exist_ok=True)
def fetch(iv):
 ...
 return out
results=list(concurrent.futures.ThreadPoolExecutor(max_workers=8).map(fetch,[(i,rows[i]) for i in [20,21,22,23,24,26,27,28,30,31,32,33,36,37,38,39,40,42,43,44,45,48,49,50,52,53,54,55,56,57,59,60,61,63,64,66,67,68,69]]))
