...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round45-root-draft.json'))
rows=json.load(open(r/'general-round45-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={1, 2, 4, 5, 7, 8, 10, 11, 12, 14, 21, 27, 28, 29, 30, 31, 32, 33, 36, 37, 38, 40, 41, 42, 43, 44, 45, 47, 48, 49, 50, 52, 53, 54, 55, 56, 57, 58, 60, 63, 65, 66, 69, 70, 71, 72, 73, 74, 75, 76, 77, 79, 80, 81, 82, 83, 84, 86, 87, 88, 89, 90, 91, 94, 95, 96, 97, 98, 99}
...
assert len(p)==100
(r/'general-round45-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
