...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round137-input.json'));exec((r/'check_round137_incremental.py').read_text().split('p=json.load(open(root/')[0]);dest=r/'round137-image-audit-final';dest.mkdir(exist_ok=True)
def fetch(iv):
