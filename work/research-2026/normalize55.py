...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round55-input.json'));e={};p={}
for part in ['first33','second33','third34']:e.update(json.load(open(r/f'round55-{part}-evidence.json')))
for part in ['first33','second33','third34','audit','extra','ownership-extra','second-extra','root']:
 f=r/f'round55-{part}-corrections.json'
 if f.exists():
...
for i,row in enumerate(rows):
 ...
 add(i,name,desc,v.get('website',''),kind=('Organization' if v.get('kind') in ['Private company','Public company','Nonprofit / tax-exempt'] else v.get('kind','Organization')),own=own,sources=src,notes=notes,outcome=out,featured=i in [3,5,6,8,13,14,16,26,29,37,41,42,43,44,45,57,69,71,76,79,83,86,90,91,92,96,98])
(r/'general-round55-root-draft.json').write_text(json.dumps(p,indent=2)+'\n')
