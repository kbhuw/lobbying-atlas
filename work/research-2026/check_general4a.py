...
with concurrent.futures.ThreadPoolExecutor(max_workers=8) as ex:
 for k,v in ex.map(one,[(k,v) for k,v in profiles.items() if v.get("website") and k in {x["id"] for x in json.load(open("work/research-2026/general-round4-a-input.json"))}]):print(k,v.get("http_status"),v.get("error",""),flush=True)
