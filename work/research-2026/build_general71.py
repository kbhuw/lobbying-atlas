...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round71-root-draft.json'))
rows=json.load(open(r/'general-round71-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={0,1,2,3,4,5,6,7,9,10,12,13,15,17,18,19,20,21,22,23,24,26,27,28,30,33,34,37,38,39,45,46,47,52,53,56,57,58,59,61,63,65,66,68,70,71,73,74,76,77,79,80,81,82,84,85,86,87,88,89,92,93,94,97,99}
...
assert len(p)==100
(r/'general-round71-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
