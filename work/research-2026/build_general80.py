...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round80-root-draft.json'))
rows=json.load(open(r/'general-round80-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={0, 3, 4, 6, 7, 8, 9, 10, 11, 12, 15, 17, 18, 21, 22, 23, 24, 28, 31, 34, 35, 38, 39, 41, 42, 43, 44, 45, 46, 49, 50, 53, 58, 59, 60, 61, 62, 63, 64, 66, 69, 72, 73, 75, 78, 79, 80, 81, 82, 83, 84, 85, 86, 87, 89, 90, 92, 93, 95, 96, 99}
...
assert len(p)==100
(r/'general-round80-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
