...
for label in ['a','b','c']:
 for x in json.load(open(root/'college-scorecard'/f'round2-{label}-input.json')):
  v=json.load(open(cache/(x['id']+'.json')))
  if 'sequence item' in v.get('error',''):
   p[x['id']]={'name':x['location_matches'][0]['school']['INSTNM'],'website':v['requested_url']}
with concurrent.futures.ThreadPoolExecutor(max_workers=8) as ex:
 for k,v in ex.map(one,p.items()):print(k,v.get('http_status'),v.get('logo_url'),flush=True)
