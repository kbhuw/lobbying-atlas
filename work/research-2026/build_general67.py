...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round67-root-draft.json'))
rows=json.load(open(r/'general-round67-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow=set([3,4,6,8,11,14,15,16,20,23,24,29,30,31,34,35,36,38,39,41,42,43,44,47,48,49,50,54,55,57,59,60,62,63,64,65,69,72,73,75,76,78,79,80,81,83,84,85,90,91,92,93,95,96,97,99])
allow.update([2,7,21])
allow.update([13,22,25,45,52,53,61,66,86,94])
...
assert len(p)==100
(r/'general-round67-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
