...
r=pathlib.Path('work/research-2026');exec((r/'check_general68_final.py').read_text().split('p=json.load(open(root/')[0]);a=json.load(open(r/'round68-logo-fixes.json'));rows=json.load(open(r/'general-round68-input.json'));index={x['id']:i for i,x in enumerate(rows)}
dest=r/'round68-image-audit';canvas=Image.new('RGB',(1200,1800),'#d6d6d6');d=ImageDraw.Draw(canvas);ok=[]
