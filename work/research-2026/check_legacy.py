...
a=json.load(open(root/"legacy-2026-audit.json"));items=[(x["id"],x["existing_profile"]) for x in a["profiles"] if x["existing_profile"].get("website")]
with concurrent.futures.ThreadPoolExecutor(max_workers=6) as ex:
 for k,v in ex.map(one,items):print(k,v.get("http_status",v.get("error")),v.get("logo_kind"),flush=True)
