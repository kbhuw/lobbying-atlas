...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round55-root-draft.json'))
rows=json.load(open(r/'general-round55-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={71,74,40,53,54,60,64,97,9,25,34,62,65,86,0,1,5,6,8,10,12,13,14,15,16,17,18,19,22,23,29,32,33,38,41,42,43,45,52,57,58,61,63,66,67,69,70,72,73,75,76,77,80,81,82,83,85,87,88,90,91,92,93,95,96,98}
...
assert len(p)==100
(r/'general-round55-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
