...
r=pathlib.Path('work/research-2026');site=pathlib.Path('lobbying-map');p=json.load(open(r/'reviewed.json'));batch=json.load(open(r/'resolved-foundation-profiles.json'));holds=json.load(open(site/'research/registry-match-holds.json'));assert len(batch)==8
p.update(batch)
assert len(p)==419
...
if not any(b['name']=='resolved-foundation-8' for b in m['batches']):m['batches'].append({'name':'resolved-foundation-8','ids':sorted(batch)})
(site/'research/reviewed-2026-manifest.json').write_text(json.dumps(m,indent=2)+'\n')
f=site/'scripts/test-reviewed-2026.mjs';f.write_text(f.read_text().replace('length,411','length,419').replace('411 exact IDs','419 exact IDs'))
