...
R=pathlib.Path(__file__).resolve().parent;ROOT=R.parents[2];master=ROOT/'work/research-2026/reviewed.json';p=json.load(open(master));shutil.copyfile(master,R/'agent30-before.json');q=json.load(open(R/'matches.json'))[:30];audit=[]
for item in q:
 i=item['id'];x=p[i];m=item['matches'][0];e=m['registration']
 if i in ['8863c3070bafba13','b502442cb751c97c','8abe27bf555f5f63','5fda87dfc45483d1','f32f98dd9f513a71','a18bcaf8d3e4527a','ce70d02892a369f4','28355d148d684068']:
...
(R/'agent30-decisions.json').write_text(json.dumps(audit,indent=2,ensure_ascii=False)+'\n');print('22 additional identities confirmed;8 retained partial.')
