...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round79-root-draft.json'))
rows=json.load(open(r/'general-round79-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={0, 1, 2, 5, 6, 8, 11, 12, 13, 14, 15, 17, 18, 19, 20, 21, 22, 23, 24, 25, 27, 28, 29, 30, 32, 34, 35, 38, 39, 40, 45, 48, 49, 50, 53, 54, 55, 57, 59, 61, 62, 63, 68, 71, 72, 73, 74, 75, 76, 77, 78, 80, 81, 83, 85, 86, 89, 92, 99}
allow.update({69,87})
...
assert len(p)==100
(r/'general-round79-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
