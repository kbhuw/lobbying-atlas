...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round52-input.json'));e={};p={}
for part in ['first33','second33','third34']:e.update(json.load(open(r/f'round52-{part}-evidence.json')))
for part in ['first33','second33','third34','audit','extra','ownership-extra','root']:
 f=r/f'round52-{part}-corrections.json'
 if f.exists():
...
for i,row in enumerate(rows):
 ...
 add(i,name,desc,v.get('website',''),kind=('Organization' if v.get('kind') in ['Private company','Public company','Nonprofit / tax-exempt'] else v.get('kind','Organization')),own=own,sources=src,notes=notes,outcome=out,featured=i in [9,11,12,14,15,16,25,26,27,31,40,41,42,43,71,72,73,74,75,76,79,83,84,90])
(r/'general-round52-root-draft.json').write_text(json.dumps(p,indent=2)+'\n')
