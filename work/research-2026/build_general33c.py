...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round33c-root-draft.json'))
rows=json.load(open(r/'general-round33-c-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
hold={rows[i]['id'] for i in [1,2,4,6,8,9,16,17,20,21]}
for k,v in p.items():
...
assert len(p)==30
(r/'general-round33c-30-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
