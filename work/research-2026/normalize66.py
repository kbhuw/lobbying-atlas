...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round66-input.json'));e={};p={}
for part,start,end in [('first33',0,33),('second33',33,66),('third34',66,100)]:
 data=json.load(open(r/f'round66-{part}-evidence.json'))
...
for part in ['first33-audit','second33-audit','third34-audit','first33-extra','second33-extra','third34-extra','second33-final','logo-root','root']:
 f=r/f'round66-{part}-corrections.json'
 if f.exists():
...
for i,row in enumerate(rows):
 ...
 add(i,name,desc,v.get('website') or '',kind=('Organization' if v.get('kind') in ['Private company','Public company','Nonprofit / tax-exempt'] else v.get('kind','Organization')),own=own,sources=src,notes=notes,outcome=out,featured=i in [12,17,18,25,26,27,33,40,44,46,47,48,55,56,57,58,62,63,78,85,89,90,91,98])
(r/'general-round66-root-draft.json').write_text(json.dumps(p,indent=2)+'\n')
