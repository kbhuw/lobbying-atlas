...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round47-input.json'));e={};p={}
for part in ['first33','second33','third34']:e.update(json.load(open(r/f'round47-{part}-evidence.json')))
for part in ['first33','second33','third34','audit','extra','ownership-extra','root']:
 f=r/f'round47-{part}-corrections.json'
 if f.exists():
...
for i,row in enumerate(rows):
 ...
 add(i,name,desc,v.get('website',''),kind=('Organization' if v.get('kind') in ['Private company','Public company','Nonprofit / tax-exempt'] else v.get('kind','Organization')),own=own,sources=src,notes=notes,outcome=out,featured=i in [14,16,18,26,27,32,37,49,52,53,54,61,66,71,74,75,81,82,85,86,90,92,93,98])
(r/'general-round47-root-draft.json').write_text(json.dumps(p,indent=2)+'\n')
