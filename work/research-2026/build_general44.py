...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round44-root-draft.json'))
rows=json.load(open(r/'general-round44-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={1,2,4,5,6,7,8,10,12,14,17,22,23,24,27,30,32,33,34,35,36,38,40,42,47,48,51,53,55,56,57,58,59,60,61,62,63,64,65,66,67,68,70,71,72,73,74,75,76,77,78,79,81,82,84,86,90,91,92,93,94,96,97}
...
assert len(p)==100
(r/'general-round44-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
