...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round59-root-draft.json'))
rows=json.load(open(r/'general-round59-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={31,39,46,58,68,38,50,69,71,75,84,97,98,99,1,3,4,5,6,7,8,10,11,12,13,14,15,16,18,19,22,23,24,25,27,30,32,33,34,35,41,42,43,44,45,48,49,51,52,53,54,55,56,57,59,61,63,65,66,67,70,72,73,74,76,77,78,81,82,88,89,90,91,92,93,94,95,96}
...
assert len(p)==100
(r/'general-round59-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
