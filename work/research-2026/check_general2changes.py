...
profiles=json.load(open("work/research-2026/general-round2-90-rootchecked.json"))
items=[]
for k,v in profiles.items():
 f=cache/f"{k}.json"
 c=json.load(open(f)) if f.exists() else {}
 if v.get("website") and c.get("requested_url")!=v["website"]:items.append((k,v))
with concurrent.futures.ThreadPoolExecutor(max_workers=8) as ex:
 for k,v in ex.map(one,items):print(k,v.get("http_status"),v.get("error",""),flush=True)
