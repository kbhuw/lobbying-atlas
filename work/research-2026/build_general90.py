...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round90-root-draft.json'))
...
allow=set(json.load(open(r/'round90-logo-review.json'))['approved_indices'])
reviewed_assets={x['id']:x.get('url','') for x in json.load(open(r/'round90-image-audit-final/fetches.json'))}
...
assert len(p)==100
(r/'general-round90-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
