...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round54-root-draft.json'))
rows=json.load(open(r/'general-round54-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={4,12,0,1,2,5,7,9,10,11,19,20,21,22,23,24,25,27,30,31,33,35,36,37,40,42,44,46,47,48,50,51,52,56,57,58,60,61,62,63,64,65,66,67,68,69,70,71,72,73,74,75,76,77,78,80,81,83,84,85,86,87,89,90,92,94,95,96,97,98,99}
...
assert len(p)==100
(r/'general-round54-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
