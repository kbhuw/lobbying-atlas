...
p=json.load(open(root/"irs-round3-rootchecked-draft.json")); todo=[]
for k,v in p.items():
 cp=cache/(k+".json");c=json.load(open(cp)) if cp.exists() else {}
 if v.get("website") and c.get("requested_url", "").lower().rstrip("/")!=v["website"].lower().rstrip("/"):todo.append((k,v))
