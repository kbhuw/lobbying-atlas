...
r=pathlib.Path('work/research-2026');site=pathlib.Path('lobbying-map');p=json.load(open(r/'reviewed.json'));batch=json.load(open(r/'college-scorecard/round2-rootchecked.json'));holds=json.load(open(site/'research/registry-match-holds.json'));assert len(batch)==189
p.update(batch)
assert len(p)==1173
...
if not any(b['name']=='college-scorecard-next189' for b in m['batches']):m['batches'].append({'name':'college-scorecard-next189','ids':sorted(batch)})
(site/'research/reviewed-2026-manifest.json').write_text(json.dumps(m,indent=2)+'\n')
f=site/'scripts/test-reviewed-2026.mjs';f.write_text(f.read_text().replace('length,984','length,1173').replace('984 exact IDs','1173 exact IDs'))
