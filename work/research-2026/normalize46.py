...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round46-input.json'));e={};p={}
for part in ['first33','second33','third34']:e.update(json.load(open(r/f'round46-{part}-evidence.json')))
for part in ['first33','second33','third34','audit','root']:
 f=r/f'round46-{part}-corrections.json'
 if f.exists():
...
for i,row in enumerate(rows):
 ...
 add(i,name,desc,v.get('website',''),kind=('Organization' if v.get('kind') in ['Private company','Public company','Nonprofit / tax-exempt'] else v.get('kind','Organization')),own=own,sources=src,notes=notes,outcome=out,featured=i in [6,7,8,11,13,14,15,20,22,24,25,26,28,30,31,32,33,36,37,39,41,43,44,45,46,48,52,54,56,57,58,59,60,64,65,66,68,69,70,71,72,74,76,78,82,84,93,94,96,98,99])
(r/'general-round46-root-draft.json').write_text(json.dumps(p,indent=2)+'\n')
