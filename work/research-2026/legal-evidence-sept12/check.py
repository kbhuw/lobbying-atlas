...
w=pathlib.Path('work/research-2026');d=json.loads((w/'reviewed.json').read_text());out=w/'legal-evidence-sept12';out.mkdir(exist_ok=True)
queue=json.loads((w/'exact-identity-source-queue-sept12.json').read_text());rows=[dict(id=r['id'],profile=d[r['id']],selected_source=r['sources'][0]) for r in queue]
...
def run(r):
 ...
 try:
  ...
  matches=list(re.finditer(r'incorporated|corporation|limited|subsidiary|legal|operated by|doing business|formerly',txt,re.I));z['snippets']=[txt[max(0,m.start()-160):m.end()+200] for m in matches[:8]];z['status']='fetched'
