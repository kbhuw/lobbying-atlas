...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round43-root-draft.json'))
rows=json.load(open(r/'general-round43-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={0,1,4,5,6,7,9,10,13,14,15,16,17,21,22,24,25,28,29,30,31,32,33,35,37,38,39,40,41,42,44,45,46,48,50,51,52,53,54,55,58,59,61,62,63,64,65,66,67,68,71,73,75,76,77,78,80,81,84,86,87,89,92,93,94,95,97,99}
...
assert len(p)==100
(r/'general-round43-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
