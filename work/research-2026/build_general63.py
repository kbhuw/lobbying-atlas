...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round63-root-draft.json'))
rows=json.load(open(r/'general-round63-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={0, 2, 5, 7, 8, 9, 10, 12, 13, 17, 21, 22, 23, 24, 25, 26, 29, 30, 33, 34, 35, 36, 38, 41, 42, 43, 45, 46, 47, 48, 50, 51, 52, 53, 54, 56, 57, 58, 60, 61, 62, 63, 64, 66, 67, 72, 74, 75, 77, 78, 80, 81, 82, 83, 84, 85, 86, 87, 88, 89, 90, 91, 93, 94, 95, 96, 97, 98, 99}
...
assert len(p)==100
(r/'general-round63-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
