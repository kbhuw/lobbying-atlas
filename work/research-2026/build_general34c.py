...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round34c-root-draft.json'))
rows=json.load(open(r/'general-round34-c-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={2,4,5,6,8,9,10,12,13,14,15,16,17,19,20,22,23,24,25,26,27,30,31,33,34,35,36,37,39,41,42,44,45,47,48,49,50,52,53,55,56,58,59,60,62,63,64,65,69,70,72,73,74,75,80,81,82,85,86,87,88,90,91,92,98,99}
...
assert len(p)==100
(r/'general-round34c-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
