...
p=pathlib.Path('work/research-2026');d=p/'government119-root-cache';d.mkdir(exist_ok=True)
xs=json.load(open(p/'government119-input.json'))[40:50]
def f(x):
...
(p/'government119-root-fetch.json').write_text(json.dumps(out,indent=2));print(json.dumps(out,indent=2))
