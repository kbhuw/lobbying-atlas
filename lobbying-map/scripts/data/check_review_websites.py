...
profiles=json.load(open('lobbying-map/research/profiles.json'));sample={x['id'] for x in json.load(open(root/'sample.json'))}
for f in root.glob('output-?.json'):
 for k,v in json.load(open(f)).items():
  if v.get('website_status')=='verified':profiles[k]=v
items=[(k,p) for k,p in profiles.items() if p.get('website')]
