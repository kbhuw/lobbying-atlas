...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round59-input.json'));e={};p={}
for part in ['first33','second33','third34']:e.update(json.load(open(r/f'round59-{part}-evidence.json')))
for part in ['first33-audit','second33-audit','third34-audit','first33-extra','second33-extra','third34-extra','root']:
 f=r/f'round59-{part}-corrections.json'
 if f.exists():
...
for i,row in enumerate(rows):
 ...
 add(i,name,desc,v.get('website') or '',kind=('Organization' if v.get('kind') in ['Private company','Public company','Nonprofit / tax-exempt'] else v.get('kind','Organization')),own=own,sources=src,notes=notes,outcome=out,featured=i in [1,3,5,6,13,14,15,19,21,23,24,25,26,31,33,35,36,38,48,54,58,60,61,63,64,66,67,69,70,71,72,73,76,81,89,91,94,95,96,97,98,99])
(r/'general-round59-root-draft.json').write_text(json.dumps(p,indent=2)+'\n')
