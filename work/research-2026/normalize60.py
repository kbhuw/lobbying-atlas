...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round60-input.json'));e={};p={}
for part,start,end in [('first33',0,33),('second33',33,66),('third34',66,100)]:
 data=json.load(open(r/f'round60-{part}-evidence.json'))
...
for part in ['first33-audit','second33-audit','third34-audit','first33-extra','second33-extra','third34-extra','root']:
 f=r/f'round60-{part}-corrections.json'
 if f.exists():
...
for i,row in enumerate(rows):
 ...
 add(i,name,desc,v.get('website') or '',kind=('Organization' if v.get('kind') in ['Private company','Public company','Nonprofit / tax-exempt'] else v.get('kind','Organization')),own=own,sources=src,notes=notes,outcome=out,featured=i in [9,10,11,16,17,18,19,21,31,36,37,39,43,46,47,48,49,50,53,54,55,59,60,61,66,70,73,75,78,81,90,93,95,96])
(r/'general-round60-root-draft.json').write_text(json.dumps(p,indent=2)+'\n')
