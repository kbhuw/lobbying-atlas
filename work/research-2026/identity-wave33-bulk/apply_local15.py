...
R=pathlib.Path(__file__).resolve().parent;ROOT=R.parents[2];master=ROOT/'work/research-2026/reviewed.json';p=json.load(open(master));shutil.copyfile(master,R/'local15-before.json');q=json.load(open(R/'matches.json'))[50:65];audit=[]
for item in q:
 i=item['id'];x=p[i];m=item['matches'][0];e=m['registration']
 if i in ['4902abe99a0df562','3185f8256b8b3829','f3ffe658ae0b8264','d22d28c8c9e30c53']:
 ...
 if i=='676e9746a14bcdff':x['notes']='Official footer explicitly identifies Dignity Gold, LLC as website operator, and contact address matches the filing. Equity ownership remains unverified.'
