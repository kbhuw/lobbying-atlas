...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round69-input.json'));e={};p={}
for part,start,end in [('first33',0,33),('second33',33,66),('third34',66,100)]:
 data=json.load(open(r/f'round69-{part}-evidence.json'))
...
for part in ['first33-audit','second33-audit','third34-audit','first33-extra','second33-extra','third34-extra','second33-final','third34-final','logo-root','root']:
 f=r/f'round69-{part}-corrections.json'
 if f.exists():
...
for i,row in enumerate(rows):
 ...
 add(i,name,desc,v.get('website') or '',kind=('Organization' if v.get('kind') in ['Private company','Public company','Nonprofit / tax-exempt'] else v.get('kind','Organization')),own=own,sources=src,notes=notes,outcome=out,featured=i in [1,5,10,13,15,16,17,18,19,20,21,22,25,27,28,33,34,35,37,38,39,40,41,43,45,47,50,53,60,69,74,76,77,78,79,80,82,83,84,86,87,88,90,93,98])
(r/'general-round69-root-draft.json').write_text(json.dumps(p,indent=2)+'\n')
