...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round39-root-draft.json'))
rows=json.load(open(r/'general-round39-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={57,46,52,59,61,73,0,2,3,4,6,7,8,10,11,12,13,14,15,18,20,21,22,23,27,28,31,34,35,37,38,39,42,44,45,47,48,49,50,51,53,54,55,56,58,60,62,65,66,67,69,71,76,77,78,80,81,82,83,85,86,87,88,89,90,93,95,96,98,99}
...
assert len(p)==100
(r/'general-round39-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
