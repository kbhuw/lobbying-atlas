...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round58-input.json'));e={};p={}
for part in ['first33','second33','third34']:e.update(json.load(open(r/f'round58-{part}-evidence.json')))
for part in ['first33-audit','second33-audit','third34-audit','first33-extra','second33-extra','third34-extra','final-extra','ownership-final','root']:
 f=r/f'round58-{part}-corrections.json'
 if f.exists():
...
for i,row in enumerate(rows):
 ...
 add(i,name,desc,v.get('website') or '',kind=('Organization' if v.get('kind') in ['Private company','Public company','Nonprofit / tax-exempt'] else v.get('kind','Organization')),own=own,sources=src,notes=notes,outcome=out,featured=i in [1,2,3,6,8,13,15,27,29,34,36,38,43,44,45,48,50,51,52,53,55,58,65,66,67,68,69,72,73,75,79,80,83,84,88,89,90,91,97,99])
(r/'general-round58-root-draft.json').write_text(json.dumps(p,indent=2)+'\n')
