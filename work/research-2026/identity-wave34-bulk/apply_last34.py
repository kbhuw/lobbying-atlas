...
R=pathlib.Path(__file__).resolve().parent;ROOT=R.parents[2];master=ROOT/'work/research-2026/reviewed.json';p=json.load(open(master));shutil.copyfile(master,R/'last34-before.json');q=json.load(open(R/'matches.json'))[70:];audit=[]
for item in q:
 i=item['id'];x=p[i];m=item['matches'][0];e=m['registration']
 if i in ['bcc5ff75d751a526','201fb18a3a84520d','ae44ca7520c4f426','d45d41ef8e95e491','1ba04720d7c18984']:
...
(R/'last34-decisions.json').write_text(json.dumps(audit,indent=2,ensure_ascii=False)+'\n');print('29 additional identities confirmed;5 retained partial.')
