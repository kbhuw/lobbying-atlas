...
R=pathlib.Path(__file__).resolve().parent;ROOT=R.parents[2];master=ROOT/'work/research-2026/reviewed.json';p=json.load(open(master));shutil.copyfile(master,R/'local20-before.json');q=json.load(open(R/'matches.json'))[30:50];audit=[]
for item in q:
 i=item['id'];x=p[i];m=item['matches'][0];e=m['registration']
 if i in ['4b47ff4f23e29d26','6a9e4752994e3b6e']:
...
(R/'local20-decisions.json').write_text(json.dumps(audit,indent=2,ensure_ascii=False)+'\n');print('18 additional identities confirmed;2 retained partial.')
