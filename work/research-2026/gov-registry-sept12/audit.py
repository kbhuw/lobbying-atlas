...
for k,v in cs.items():
 ...
 if not reasons:
  ...
  if norm(v['name'])!=norm(r['Organization name']):reasons.append('Profile and registered legal name differ')
  proof=None
  if not reasons:
   for s in v['sources']:
...
o=ROOT/'gov-registry-sept12';o.mkdir(exist_ok=True);(o/'audit.json').write_text(json.dumps({'accepted':accepted,'deferred':deferred},indent=2)+'\n')
