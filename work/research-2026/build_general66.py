...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round66-root-draft.json'))
rows=json.load(open(r/'general-round66-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow=set([0, 1, 3, 4, 5, 7, 8, 9, 11, 12, 14, 16, 17, 18, 19, 21, 22, 23, 24, 26, 27, 29, 30, 31, 33, 34, 37, 38, 39, 41, 43, 44, 45, 46, 47, 48, 50, 52, 53, 54, 56, 57, 58, 59, 62, 63, 66, 70, 71, 73, 74, 75, 76, 77, 78, 80, 81, 82, 83, 84, 85, 86, 87, 88, 89, 90, 91, 93, 95, 96])
allow.update([15,40,49,60,64,65])
...
assert len(p)==100
(r/'general-round66-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
