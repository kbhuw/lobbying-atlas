...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round74-input.json'));e={};p={}
for part,start,end in [('first33',0,33),('second33',33,66),('third34',66,100)]:
 data=json.load(open(r/f'round74-{part}-evidence.json'))
...
for part in ['first33-audit','second33-audit','third34-audit','first33-extra','second33-extra','third34-extra','second33-final','third34-final','logo-root','root']:
 f=r/f'round74-{part}-corrections.json'
 if f.exists():
...
(r/'general-round74-root-draft.json').write_text(json.dumps(p,indent=2)+'\n')
