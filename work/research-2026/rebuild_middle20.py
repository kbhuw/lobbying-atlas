...
r=pathlib.Path('work/research-2026');rows=[line.split('\t',2) for line in (r/'root-middle20-descriptions.tsv').read_text().splitlines()];descriptions={k:d for k,n,d in rows};names={k:n for k,n,d in rows};out={};holds=json.load(open('lobbying-map/research/registry-match-holds.json'))
# Root checked the cached page identities; redirects to renamed/ambiguous organizations remain filed.
ambiguous={'a15f349c15ec29e2'}
for x in json.load(open(r/'root-middle20-full.json')):
 k=x['id']
 ...
 out[k]=v
assert len(out)==20
(r/'root-middle20-reviewed.json').write_text(json.dumps(out,indent=2)+'\n');print('Reviewed',len(out),'ID-keyed records.')
