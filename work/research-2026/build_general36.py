...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round36-root-draft.json'))
rows=json.load(open(r/'general-round36-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={1, 2, 3, 5, 6, 7, 9, 10, 11, 15, 18, 19, 20, 22, 23, 26, 28, 29, 30, 33, 37, 40, 41, 49, 54, 58, 60, 61, 62, 64, 65, 66, 67, 68, 69, 73, 75, 79, 81, 82, 83, 84, 85, 88, 89, 91, 92, 94, 96, 97, 99}
allow.update({41,50,64,76,78})
...
assert len(p)==100
(r/'general-round36-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
