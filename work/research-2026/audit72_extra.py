...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round72-input.json'));exec((r/'check_general72_final.py').read_text().split('p=json.load(open(root/')[0]);dest=r/'round72-image-audit';dest.mkdir(exist_ok=True)
def fetch(iv):
 ...
 return out
results=list(concurrent.futures.ThreadPoolExecutor(max_workers=8).map(fetch,[(i,rows[i]) for i in [8,11,12,13,21,29,33,34,35,36,37,38,49,52,53,56,58,66,72,78,79,81,82,85,96]]))
canvas=Image.new('RGB',(1600,1490),'#d6d6d6');d=ImageDraw.Draw(canvas)
for z in results:
...
canvas.save(dest/'extra-sheet.png');(dest/'extra-fetches.json').write_text(json.dumps(results,indent=2)+'\n');print('Image audit sheet ready')
