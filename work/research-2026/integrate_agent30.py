...
r=pathlib.Path('work/research-2026');site=pathlib.Path('lobbying-map');p=json.load(open(r/'reviewed.json'));batch=json.load(open(r/'next-agent30-rootchecked.json'));holds=json.load(open(site/'research/registry-match-holds.json'));assert len(batch)==30
p.update(batch)
p.update(json.load(open(r/"brand-relationship-followups.json")))
assert len(p)==489
...
if not any(b['name']=='agent-30-root-checked' for b in m['batches']):m['batches'].append({'name':'agent-30-root-checked','ids':sorted(batch)})
(site/'research/reviewed-2026-manifest.json').write_text(json.dumps(m,indent=2)+'\n')
f=site/'scripts/test-reviewed-2026.mjs';f.write_text(f.read_text().replace('length,459','length,489').replace('459 exact IDs','489 exact IDs'))
