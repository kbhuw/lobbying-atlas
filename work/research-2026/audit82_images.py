...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round82-input.json'));exec((r/'check_round82_incremental.py').read_text().split('p=json.load(open(root/')[0]);dest=r/'round82-image-audit-final';dest.mkdir(exist_ok=True)
def fetch(iv):
