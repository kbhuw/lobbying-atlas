...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round43-input.json'));e={};p={}
for part in ['first33','second33','third34']:e.update(json.load(open(r/f'round43-{part}-evidence.json')))
for part in ['first33','second33','third','first-audit','cross-audit','audit','root']:
 f=r/f'round43-{part}-corrections.json'
 if f.exists():
...
for i,row in enumerate(rows):
 ...
 add(i,name,desc,v.get('website',''),kind=('Organization' if v.get('kind') in ['Private company','Public company','Nonprofit / tax-exempt'] else v.get('kind','Organization')),own=own,sources=src,notes=notes,outcome=out,featured=i in [0,1,4,5,6,10,11,14,15,17,24,25,32,37,44,48,51,52,53,58,63,68,69,77,80,87,99])
(r/'general-round43-root-draft.json').write_text(json.dumps(p,indent=2)+'\n')
