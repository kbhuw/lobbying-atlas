...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round58-root-draft.json'))
rows=json.load(open(r/'general-round58-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={1,8,78,41,53,55,56,59,63,98,0,2,3,4,5,6,7,9,10,11,13,14,15,18,23,27,28,29,33,34,35,36,38,43,44,46,47,49,50,51,52,54,61,62,64,65,66,67,68,69,70,71,72,73,75,79,80,81,82,83,84,85,87,88,89,90,91,92,93,97,99}
...
assert len(p)==100
(r/'general-round58-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
