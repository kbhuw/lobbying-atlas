...
r=pathlib.Path('work/research-2026');site=pathlib.Path('lobbying-map');p=json.load(open(r/'reviewed.json'));batch=json.load(open(r/'public-eight.json'));holds=json.load(open(site/'research/registry-match-holds.json'));assert len(batch)==8
p.update(batch)
assert len(p)==429
...
if not any(b['name']=='public-companies-8' for b in m['batches']):m['batches'].append({'name':'public-companies-8','ids':sorted(batch)})
(site/'research/reviewed-2026-manifest.json').write_text(json.dumps(m,indent=2)+'\n')
f=site/'scripts/test-reviewed-2026.mjs';f.write_text(f.read_text().replace('length,421','length,429').replace('421 exact IDs','429 exact IDs'))
