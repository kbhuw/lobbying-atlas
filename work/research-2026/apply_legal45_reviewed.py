...
p=Path('work/research-2026');f=p/'reviewed.json';d=json.loads(f.read_text());a=json.loads((p/'legal-pages45-reviewed.json').read_text());cs={r['id']:r for r in json.loads((p/'relaxed-address-candidates42.json').read_text())};before={};audit=[];now=datetime.datetime.now(datetime.timezone.utc).isoformat()
for r in a:
 id=r['id'];
