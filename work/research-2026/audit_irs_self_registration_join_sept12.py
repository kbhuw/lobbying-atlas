import csv,json,re,pathlib,zipfile,collections,sqlite3,xml.etree.ElementTree as ET
...
exclude={r['id'] for r in json.loads((ROOT/'identity-evidence-next50-sept12/queue.json').read_text())};cs={k:v for k,v in p.items() if v.get('review_outcome') in {'partial','unresolved'} and k not in exclude}
irs={};frequency=collections.Counter();byname=collections.defaultdict(list)
needed={norm(v['name']) for v in cs.values()}
for f in pathlib.Path('work/organization-research/irs').glob('eo*.csv'):
 with f.open() as z:
  for r in csv.DictReader(z):
   ...
   if norm(r['NAME']) in needed:byname[norm(r['NAME'])].append(r)
cs={k:v for k,v in cs.items() if norm(v['name']) in byname}
db=sqlite3.connect('file:work/organization-research/bulk-profiles.sqlite?mode=ro',uri=True);db.row_factory=sqlite3.Row
registration_sources=collections.defaultdict(list)
for row in db.execute('SELECT name,city,state,source_zip,source_member,source_url FROM descriptions'):
 if norm(row['name']) in needed:
  registration_sources[norm(row['name'])].append({'url':row['source_url'],'label':row['source_member'],'claim':''})
db.close()
zs={};accepted=[];deferred=[]
for k,v in cs.items():
 reasons=[];matches=[]
 matches=byname[norm(v['name'])]
 ...
 if not reasons:
  ...
  proof=None
  if not reasons:
   for s in v['sources']+registration_sources[norm(v['name'])]:
    ...
    for m in set(members):
     ...
     name,city,state=val('clientName'),val('clientCity'),val('clientState');address_source='client'
     if val('selfSelect')=='Y' and norm(val('organizationName'))==norm(name) and not city and not state:
      city,state=val('city'),val('state');address_source='self-registrant'
     if (norm(name),norm(city),state)==key:
      proof={'archive':str(f),'member':names[m],'client_name':name,'city':city,'state':state,'source_url':s['url'],'address_source':address_source,'selfSelect':val('selfSelect')};break
    if proof:break
...
o=ROOT/'irs-self-registration-join-sept12';o.mkdir(exist_ok=True);(o/'audit.json').write_text(json.dumps({'accepted':accepted,'deferred':deferred},indent=2)+'\n')
