...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round86-input.json'));exec((r/'check_round86_incremental.py').read_text().split('p=json.load(open(root/')[0]);dest=r/'round86-image-audit-final';dest.mkdir(exist_ok=True)
def fetch(iv):
