...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round54-input.json'));e={};p={}
for part in ['first33','second33','third34']:e.update(json.load(open(r/f'round54-{part}-evidence.json')))
for part in ['first33','second33','third34','audit','extra','ownership-extra','second-extra','root']:
 f=r/f'round54-{part}-corrections.json'
 if f.exists():
...
for i,row in enumerate(rows):
 ...
 add(i,name,desc,v.get('website',''),kind=('Organization' if v.get('kind') in ['Private company','Public company','Nonprofit / tax-exempt'] else v.get('kind','Organization')),own=own,sources=src,notes=notes,outcome=out,featured=i in [3,4,5,15,17,20,21,24,25,27,29,31,39,41,42,43,46,51,55,58,60,63,65,66,68,70,71,72,73,75,76,79,81,85,86,88,96,97,98,99])
(r/'general-round54-root-draft.json').write_text(json.dumps(p,indent=2)+'\n')
