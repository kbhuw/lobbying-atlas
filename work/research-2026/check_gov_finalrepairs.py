...
p={}
for fn in ['three-program-domain-repairs.json','two-program-domain-repairs.json','last-two-repairs.json']:
 a=json.load(open(root/'dotgov'/fn));a={v['id']:v for v in a} if isinstance(a,list) else a;p.update(a)
with concurrent.futures.ThreadPoolExecutor(max_workers=7) as ex:
 for k,v in ex.map(one,p.items()):print(k,v.get('http_status',v.get('error')),v.get('logo_url'),flush=True)
