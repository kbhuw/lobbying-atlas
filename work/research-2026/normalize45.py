...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round45-input.json'));e={};p={}
for part in ['first33','second33','third34']:e.update(json.load(open(r/f'round45-{part}-evidence.json')))
for part in ['first33','second33','third34','audit','root']:
 f=r/f'round45-{part}-corrections.json'
 if f.exists():
...
for i,row in enumerate(rows):
 ...
 add(i,name,desc,v.get('website',''),kind=('Organization' if v.get('kind') in ['Private company','Public company','Nonprofit / tax-exempt'] else v.get('kind','Organization')),own=own,sources=src,notes=notes,outcome=out,featured=i in [2,4,5,9,11,15,16,23,24,31,36,37,39,40,41,43,44,45,46,49,50,59,60,69,71,72,73,74,76,78,79,80,82,83,84,86,87,89,90,93,95,96,98,99])
(r/'general-round45-root-draft.json').write_text(json.dumps(p,indent=2)+'\n')
