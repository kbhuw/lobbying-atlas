...
r=pathlib.Path('work/research-2026');site=pathlib.Path('lobbying-map');p=json.load(open(r/'reviewed.json'));batch=json.load(open(r/'next47-staged.json'));holds=json.load(open(site/'research/registry-match-holds.json'));assert len(batch)==47
p.update(batch)
assert len(p)==789
...
if not any(b['name']=='legacy27-and-sec20' for b in m['batches']):m['batches'].append({'name':'legacy27-and-sec20','ids':sorted(batch)})
(site/'research/reviewed-2026-manifest.json').write_text(json.dumps(m,indent=2)+'\n')
f=site/'scripts/test-reviewed-2026.mjs';f.write_text(f.read_text().replace('length,742','length,789').replace('742 exact IDs','789 exact IDs'))
