...
r=pathlib.Path('work/research-2026');d=r/'verified-sites130-root-cache';d.mkdir(exist_ok=True)
x=json.load(open(r/'verified-sites130-root-input.json'))
def f(x):
 u=x['profile']['website']
 try:
...
(r/'verified-sites130-root-fetch.json').write_text(json.dumps(y,indent=2));print(json.dumps(y))
