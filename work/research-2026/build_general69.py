...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round69-root-draft.json'))
rows=json.load(open(r/'general-round69-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={9,2,24,30,40,68,95,0,3,4,6,7,8,10,13,14,15,18,20,22,23,26,27,28,31,32,33,34,35,36,37,38,39,41,44,46,47,48,49,50,51,55,57,58,59,60,61,64,65,67,71,72,73,74,75,76,77,78,79,81,82,83,85,86,87,88,90,91,92,93,97,98,99}
...
assert len(p)==100
(r/'general-round69-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
