...
p=json.load(open(root/"irs-round4-325-root-draft.json")); todo=[]
for k,v in p.items():
 cp=cache/(k+".json");c=json.load(open(cp)) if cp.exists() else {}
 if v.get("website") and (c.get("requested_url", "").lower().rstrip("/")!=v["website"].lower().rstrip("/") or "SSL" in str(c.get("error",""))):todo.append((k,v))
print("Checking",len(todo),flush=True)
with concurrent.futures.ThreadPoolExecutor(max_workers=8) as ex:
 for k,v in ex.map(one,todo):print(k,v.get("http_status",v.get("error")),flush=True)
