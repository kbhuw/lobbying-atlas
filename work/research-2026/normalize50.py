...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round50-input.json'));e={};p={}
for part in ['first33','second33','third34']:e.update(json.load(open(r/f'round50-{part}-evidence.json')))
for part in ['first33','second33','third34','audit','extra','ownership-extra','root']:
 f=r/f'round50-{part}-corrections.json'
 if f.exists():
...
for i,row in enumerate(rows):
 ...
 add(i,name,desc,v.get('website',''),kind=('Organization' if v.get('kind') in ['Private company','Public company','Nonprofit / tax-exempt'] else v.get('kind','Organization')),own=own,sources=src,notes=notes,outcome=out,featured=i in [10,11,20,25,26,28,30,34,35,36,37,38,39,41,44,45,52,54,62,63,64,65,71,72,77,78,81,83,89,90,92,93,95,96,97])
(r/'general-round50-root-draft.json').write_text(json.dumps(p,indent=2)+'\n')
