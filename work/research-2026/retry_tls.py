...
import gzip
names={c['id']:c['name'] for c in json.load(gzip.open('lobbying-map/public/data/directory-v3.json.gz'))['companies']}
items=[]
for f in cache.glob('*.json'):
 try:a=json.load(open(f))
 except ValueError:continue
 if 'SSL' in a.get('error','') or 'tlsv1' in a.get('error','').lower():items.append((a['id'],{'name':names[a['id']],'website':a['requested_url']}))
with concurrent.futures.ThreadPoolExecutor(max_workers=6) as ex:
 for id,r in ex.map(one,items):print(id,r.get('http_status',r.get('error')),r.get('logo_kind','missing'),flush=True)
