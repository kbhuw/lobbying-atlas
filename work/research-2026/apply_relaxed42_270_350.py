...
p=Path('work/research-2026'); master=p/'reviewed.json';d=json.loads(master.read_text());c=json.loads((p/'relaxed-address-candidates42.json').read_text());a=json.loads((p/'relaxed-address42-agent-270-350.json').read_text())
accept={277,278,281,297,299,318,332,343}; backup={};audit=[];now=datetime.datetime.now(datetime.timezone.utc).isoformat()
for r in a:
 i=r['original_index']
...
(p/'relaxed42-270-350-before.json').write_text(json.dumps(backup,indent=2,ensure_ascii=False)+'\n');(p/'relaxed42-270-350-decisions.json').write_text(json.dumps(audit,indent=2,ensure_ascii=False)+'\n')
