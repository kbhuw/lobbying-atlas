...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round151-root-draft.json'))
rows=json.load(open(r/'general-round151-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow=set(json.load(open(r/'round151-logo-review.json'))['approved_indices'])
reviewed_assets={x['id']:x.get('url','') for x in json.load(open(r/'round151-image-audit-final/fetches.json'))}
hold={row['id'] for i,row in enumerate(rows) if i not in allow}
for k,v in p.items():
 v.pop('id',None);v.pop('logo_decision',None)
 v.update(as_of='2026-09-08',checked_at='2026-09-08',logo_url='',logo_kind='',logo_source_url='',logo_status='unresolved');v.setdefault('featured',False);v.setdefault('legal_form','')
 ...
 elif same and c.get('http_status')==200 and len(c.get('text',''))>151:
  v['website_status']='verified'
  if k not in hold and c.get('logo_url')==reviewed_assets.get(k) and c.get('logo_http_status')==200 and c.get('logo_url','').startswith('https://'):
...
assert len(p)==12
(r/'general-round151-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
