...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round81-root-draft.json'))
rows=json.load(open(r/'general-round81-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={0, 1, 4, 5, 6, 7, 10, 11, 13, 14, 16, 17, 18, 19, 20, 21, 22, 23, 25, 27, 31, 32, 33, 35, 37, 38, 39, 41, 42, 43, 44, 45, 48, 49, 50, 51, 52, 53, 54, 55, 56, 58, 59, 60, 65, 67, 69, 71, 75, 76, 77, 79, 80, 82, 83, 84, 87, 88, 90, 91, 92, 93}
allow.add(57)
...
assert len(p)==100
(r/'general-round81-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
