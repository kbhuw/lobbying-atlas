...
R=pathlib.Path(__file__).resolve().parent;ROOT=R.parents[2];master=ROOT/'work/research-2026/reviewed.json';p=json.load(open(master));shutil.copyfile(master,R/'agent35-before.json');q=json.load(open(R/'matches.json'))[:35];audit=[]
for item in q:
 i=item['id'];x=p[i];m=item['matches'][0];e=m['registration']
 if i in ['7732b751da32c2d3','7cae28dae4e577c8','425c1591daff084d','623439e93690c392','41373713d7cbd1ed','7e1f200d2fcfc550']:
...
(R/'agent35-decisions.json').write_text(json.dumps(audit,indent=2,ensure_ascii=False)+'\n');print('29 additional identities confirmed;6 retained partial.')
