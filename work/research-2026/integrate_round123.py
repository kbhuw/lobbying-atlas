...
r=pathlib.Path('work/research-2026');site=pathlib.Path('lobbying-map');p=json.load(open(r/'reviewed.json'));batch=json.load(open(r/'round123-reviewed.json'));holds=json.load(open(site/'research/registry-match-holds.json'));assert len(batch)==123
p.update(batch)
p.update(json.load(open(r/"compression-followups.json")))
assert len(p)==662
...
if not any(b['name']=='round-123-source-reviewed' for b in m['batches']):m['batches'].append({'name':'round-123-source-reviewed','ids':sorted(batch)})
(site/'research/reviewed-2026-manifest.json').write_text(json.dumps(m,indent=2)+'\n')
f=site/'scripts/test-reviewed-2026.mjs';f.write_text(f.read_text().replace('length,539','length,662').replace('539 exact IDs','662 exact IDs'))
