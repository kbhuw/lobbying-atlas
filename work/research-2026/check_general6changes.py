...
profiles=json.load(open("work/research-2026/general-round6-90-draft.json"))
with concurrent.futures.ThreadPoolExecutor(max_workers=8) as ex:
 for k,v in ex.map(one,[(k,v) for k,v in profiles.items() if v.get("website") and (not (cache/(k+".json")).exists() or json.load(open(cache/(k+".json"))).get("requested_url")!=v["website"])]):print(k,v.get("http_status"),v.get("error",""),flush=True)
