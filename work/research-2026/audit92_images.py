...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round92-input.json'));exec((r/'check_round92_incremental.py').read_text().split('p=json.load(open(root/')[0]);dest=r/'round92-image-audit-final';dest.mkdir(exist_ok=True)
def fetch(iv):
...
canvas=Image.new('RGB',(1600,1492),'#d6d6d6');d=ImageDraw.Draw(canvas)
for z in results:
