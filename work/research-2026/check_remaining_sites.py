...
class Page(HTMLParser):
 ...
 def handle_starttag(self,t,attrs):
  ...
  if t=='img' and re.search('logo', ' '.join(a.get(k,'') for k in ['alt','class','id','src']),re.I):
   if a.get('src') and any(t in (a.get('alt','')+' '+urllib.parse.urlparse(a['src']).path).lower() for t in self.name_tokens):self.candidates.append((0,a['src'],'logo'))
...
def get(u,maxbytes=2000000):
 ...
 with urllib.request.urlopen(req,timeout=12) as r:return r.geturl(),r.headers.get('Content-Type',''),r.read(maxbytes)
def one(item):
...
import gzip
names={c['id']:c['name'] for c in json.load(gzip.open('lobbying-map/public/data/directory-v3.json.gz'))['companies'] if c['years'].get('2026')}
profiles=json.load(open('lobbying-map/research/registry-websites.json'))
items=[(k,{'name':names[k],'website':v['website']}) for k,v in profiles.items() if k in names and not (cache/(k+'.json')).exists()]
print('Remaining website checks',len(items),flush=True)
counts={}
with concurrent.futures.ThreadPoolExecutor(max_workers=6) as ex:
 for n,(id,r) in enumerate(ex.map(one,items),1):
  status='fetched' if r.get('http_status')==200 else 'unavailable';counts[status]=counts.get(status,0)+1
  (root/'remaining-sites-progress.json').write_text(json.dumps({'processed':n,'total':len(items),'counts':counts,'updated_at':datetime.datetime.now(datetime.timezone.utc).isoformat()},indent=2))
  if n%50==0:print(n,counts,flush=True)
