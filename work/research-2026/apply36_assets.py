...
r=pathlib.Path('work/research-2026');exec((r/'check_general34a.py').read_text().split('p=json.load(open(root/')[0]);a=json.load(open(r/'round36-logo-fixes.json'));dest=r/'round36-image-audit';canvas=Image.new('RGB',(1200,400),'#d6d6d6');d=ImageDraw.Draw(canvas)
rows=json.load(open(r/'general-round36-input.json'));index={x['id']:i for i,x in enumerate(rows)};ok=[]
