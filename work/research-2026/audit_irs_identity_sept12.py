...
cs={k:v for k,v in p.items() if v.get('ownership')=='Nonprofit / tax-exempt' and v.get('review_outcome')=='partial' and any(s.get('label','').startswith('IRS organization record') for s in v['sources'])}
...
o=ROOT/'irs-identity-sept12';o.mkdir(exist_ok=True);(o/'audit.json').write_text(json.dumps({'accepted':accepted,'deferred':deferred},indent=2)+'\n')
