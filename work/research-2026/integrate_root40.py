...
r=pathlib.Path('work/research-2026');site=pathlib.Path('lobbying-map');p=json.load(open(r/'reviewed.json'));batch=json.load(open(r/'root-reviewed-40.json'));holds=json.load(open(site/'research/registry-match-holds.json'));assert len(batch)==38
p.update(batch)
assert len(p)==411
...
if not any(b['name']=='root-next-38' for b in m['batches']):m['batches'].append({'name':'root-next-38','ids':sorted(batch)})
(site/'research/reviewed-2026-manifest.json').write_text(json.dumps(m,indent=2)+'\n')
f=site/'scripts/test-reviewed-2026.mjs';f.write_text(f.read_text().replace('length,373','length,411').replace('373 exact IDs','411 exact IDs'))
