...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round77-root-draft.json'))
rows=json.load(open(r/'general-round77-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={0, 1, 2, 6, 10, 11, 12, 15, 16, 17, 18, 19, 21, 23, 24, 28, 34, 35, 38, 41, 46, 48, 52, 53, 55, 57, 61, 62, 63, 65, 67, 69, 72, 73, 74, 77, 78, 80, 82, 84, 85, 86, 89, 91, 93, 94, 95}
...
assert len(p)==100
(r/'general-round77-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
