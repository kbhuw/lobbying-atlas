...
R=pathlib.Path(__file__).resolve().parent;ROOT=R.parents[2];master=ROOT/'work/research-2026/reviewed.json';p=json.load(open(master));shutil.copyfile(master,R/'second35-before.json');q=json.load(open(R/'matches.json'))[35:70];audit=[]
for item in q:
 i=item['id'];x=p[i];m=item['matches'][0];e=m['registration']
 if i in ['5067a6c71eec7abc','cc7271b3983b45eb','d64f9e3f764a255a','7d414553168f19cc']:
...
(R/'second35-decisions.json').write_text(json.dumps(audit,indent=2,ensure_ascii=False)+'\n');print('31 additional identities confirmed;4 retained partial.')
