...
R=pathlib.Path(__file__).resolve().parent;ROOT=R.parents[2];master=ROOT/'work/research-2026/reviewed.json';p=json.load(open(master));shutil.copyfile(master,R/'late15-before.json');q=json.load(open(R/'matches.json'))[60:75];audit=[]
for item in q:
 i=item['id'];x=p[i];m=item['matches'][0];e=m['registration']
 if i=='02274a2b38022f79':continue
 if i in ['e860989c1a57fc07','ffbc26a44fad1c0d','1719e0db222f7858','aa73779c7688b0cd']:
 ...
 if 'Original research qualifier: Private company; ownership not established.' in x['notes']:x['notes']='Identity and official address confirmed; current ownership remains unverified.'
...
(R/'late15-decisions.json').write_text(json.dumps(audit,indent=2,ensure_ascii=False)+'\n');print('10 additional identities confirmed;4 retained partial;1 previously reviewed.')
