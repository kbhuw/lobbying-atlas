...
b=pathlib.Path('work/research-2026/nonprofit-assets-sept12');c=b/'logos';c.mkdir(exist_ok=True);d=json.load(open('work/research-2026/reviewed.json'));q=[]
for r in json.loads((b/'website-results.json').read_text()):
 if r['id'] not in json.loads((b/'website-approved-ids.json').read_text()) or r['status']!='fetched' or d[r['id']].get('logo_kind')=='logo' or not r.get('logo_candidates'):continue
 z=r['logo_candidates'][0]
 if not z.get('src'):continue
 if r['id'] in {'7bb5702c45402f81','897ff36a8eab9ad3','9cd385848cb2bcd9','e683c651cdfb2a4b'}:continue
