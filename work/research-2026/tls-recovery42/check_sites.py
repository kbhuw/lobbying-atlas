...
recovery=json.load(open(ROOT/'work/research-2026/remaining-work-by-evidence.json'))['queues']['recover_unreadable_official_site']
ids=set()
for x in recovery:
 d=json.load(open(ROOT/x['cache']))
 if any(page.get('error')=='SSLError' for page in d['pages']):ids.add(x['id'])
rows=[x for x in q if x['id'] in ids and p[x['id']].get('review_outcome')!='confirmed']
