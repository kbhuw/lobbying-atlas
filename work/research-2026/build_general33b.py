...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round33b-root-draft.json'))
rows=json.load(open(r/'general-round33-b-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
hold={rows[i]['id'] for i in [4,7,11,13,18,21,24,27,28]}
for k,v in p.items():
...
assert len(p)==30
(r/'general-round33b-30-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
