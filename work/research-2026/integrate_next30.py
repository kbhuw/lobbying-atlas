...
r=pathlib.Path('work/research-2026');site=pathlib.Path('lobbying-map');p=json.load(open(r/'reviewed.json'));batch=json.load(open(r/'next30-reviewed.json'));holds=json.load(open(site/'research/registry-match-holds.json'));assert len(batch)==30
p.update(batch)
assert len(p)==459
...
if not any(b['name']=='next-30-individual-reviews' for b in m['batches']):m['batches'].append({'name':'next-30-individual-reviews','ids':sorted(batch)})
(site/'research/reviewed-2026-manifest.json').write_text(json.dumps(m,indent=2)+'\n')
f=site/'scripts/test-reviewed-2026.mjs';f.write_text(f.read_text().replace('length,429','length,459').replace('429 exact IDs','459 exact IDs'))
