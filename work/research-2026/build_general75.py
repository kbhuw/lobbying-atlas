...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round75-root-draft.json'))
rows=json.load(open(r/'general-round75-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={0, 1, 2, 3, 4, 6, 8, 9, 10, 11, 12, 13, 14, 15, 21, 22, 24, 26, 27, 33, 35, 36, 38, 39, 41, 42, 44, 45, 46, 47, 49, 50, 51, 55, 56, 57, 58, 60, 61, 62, 63, 64, 65, 66, 68, 71, 72, 73, 74, 75, 76, 77, 80, 82, 85, 86, 87, 89, 90, 92, 94, 96, 97, 98}
...
assert len(p)==100
(r/'general-round75-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
