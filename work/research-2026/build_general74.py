...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round74-root-draft.json'))
rows=json.load(open(r/'general-round74-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={1,4,6,7,8,9,10,11,12,13,14,17,18,20,23,25,26,27,29,30,34,38,39,40,43,44,48,49,51,53,54,56,57,61,64,65,66,67,69,70,71,74,75,78,81,82,83,87,89,90,92,93,97,98,99}
...
assert len(p)==100
(r/'general-round74-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
