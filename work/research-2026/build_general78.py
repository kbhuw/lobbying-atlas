...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round78-root-draft.json'))
rows=json.load(open(r/'general-round78-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={0, 1, 2, 4, 6, 8, 9, 10, 13, 14, 15, 16, 17, 21, 22, 24, 25, 26, 27, 29, 30, 32, 37, 38, 40, 41, 43, 44, 45, 47, 50, 51, 53, 54, 57, 58, 59, 60, 61, 62, 63, 65, 66, 69, 70, 71, 72, 74, 75, 76, 81, 82, 86, 87, 89, 93, 96, 97, 98, 99}
...
assert len(p)==100
(r/'general-round78-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
