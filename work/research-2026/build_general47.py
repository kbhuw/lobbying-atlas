...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round47-root-draft.json'))
rows=json.load(open(r/'general-round47-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={0, 1, 2, 3, 4, 5, 6, 7, 9, 14, 15, 16, 17, 18, 19, 20, 21, 22, 24, 26, 27, 28, 32, 33, 35, 36, 37, 38, 39, 40, 41, 43, 44, 45, 46, 47, 48, 49, 52, 53, 55, 56, 58, 59, 61, 63, 64, 65, 66, 67, 70, 71, 73, 76, 77, 78, 79, 80, 81, 82, 85, 86, 89, 90, 91, 92, 93, 94, 95, 96, 97, 98, 99}
...
assert len(p)==100
(r/'general-round47-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
