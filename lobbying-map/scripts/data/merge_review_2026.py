import os, pathlib
os.chdir(pathlib.Path(__file__).resolve().parents[2].parent)
...
elif existing.with_suffix('.json.gz').exists():ps.update(json.load(gzip.open(existing.with_suffix('.json.gz'))))
...
(site/'research/reviewed-2026.json.gz').write_bytes(gzip.compress((site/'research/reviewed-2026.json').read_bytes(), mtime=0))
