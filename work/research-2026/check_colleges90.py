...
p={}
for label in ["a","b","c"]:
 for x in json.load(open(root/"college-scorecard"/f"review-{label}-input.json")):
  u=x["location_matches"][0]["school"]["INSTURL"].strip();u=u if u.startswith(("https://","http://")) else "https://"+u;p[x["id"]]={"name":x["location_matches"][0]["school"]["INSTNM"],"website":u}
with concurrent.futures.ThreadPoolExecutor(max_workers=8) as ex:
 for k,v in ex.map(one,p.items()):print(k,v.get("http_status",v.get("error")),v.get("logo_kind"),flush=True)
