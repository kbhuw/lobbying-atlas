...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round76-input.json'));exec((r/'check_round76_incremental.py').read_text().split('p=json.load(open(root/')[0]);dest=r/'round76-image-audit-remaining';dest.mkdir(exist_ok=True)
def fetch(iv):
 ...
 return out
results=list(concurrent.futures.ThreadPoolExecutor(max_workers=8).map(fetch,[(i,rows[i]) for i in [1,10,25,29,41,46,47,51,74,80,81,82,83,84,85,87,88,90,92]]))
