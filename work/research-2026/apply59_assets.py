...
r=pathlib.Path('work/research-2026');exec((r/'check_general49_final.py').read_text().split('p=json.load(open(root/')[0]);a=json.load(open(r/'round59-logo-fixes.json'));rows=json.load(open(r/'general-round59-input.json'));index={x['id']:i for i,x in enumerate(rows)}
