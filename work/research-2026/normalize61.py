...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round61-input.json'));e={};p={}
for part,start,end in [('first33',0,33),('second33',33,66),('third34',66,100)]:
 data=json.load(open(r/f'round61-{part}-evidence.json'))
...
for part in ['first33-audit','second33-audit','third34-audit','first33-extra','second33-extra','third34-extra','root']:
 f=r/f'round61-{part}-corrections.json'
 if f.exists():
...
for i,row in enumerate(rows):
 ...
 add(i,name,desc,v.get('website') or '',kind=('Organization' if v.get('kind') in ['Private company','Public company','Nonprofit / tax-exempt'] else v.get('kind','Organization')),own=own,sources=src,notes=notes,outcome=out,featured=i in [2,6,8,9,13,15,17,18,22,25,26,27,30,31,36,39,40,42,45,46,48,49,55,56,57,58,61,71,76,77,81,82,83,86,89,90,92,93,95,99])
(r/'general-round61-root-draft.json').write_text(json.dumps(p,indent=2)+'\n')
