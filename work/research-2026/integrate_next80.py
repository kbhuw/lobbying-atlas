...
r=pathlib.Path('work/research-2026');site=pathlib.Path('lobbying-map');p=json.load(open(r/'reviewed.json'));batch=json.load(open(r/'round80-next-rootchecked.json'));holds=json.load(open(site/'research/registry-match-holds.json'));assert len(batch)==80
p.update(batch)
assert len(p)==742
...
if not any(b['name']=='next-80-root-checked' for b in m['batches']):m['batches'].append({'name':'next-80-root-checked','ids':sorted(batch)})
(site/'research/reviewed-2026-manifest.json').write_text(json.dumps(m,indent=2)+'\n')
f=site/'scripts/test-reviewed-2026.mjs';f.write_text(f.read_text().replace('length,662','length,742').replace('662 exact IDs','742 exact IDs'))
