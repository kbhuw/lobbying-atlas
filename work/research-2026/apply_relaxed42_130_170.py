...
p=Path('work/research-2026'); master=p/'reviewed.json';d=json.loads(master.read_text());c=json.loads((p/'relaxed-address-candidates42.json').read_text());a=json.loads((p/'relaxed-address42-agent-130-170.json').read_text())
accept={131,133,136,142,143,144,145,150,151,152,154,155,158,159,162,166}; backup={};audit=[];now=datetime.datetime.now(datetime.timezone.utc).isoformat()
for i,r in enumerate(a,130):
 ...
 if i==144:r['legal_name_quote']='Galveston LNG Bunker Port'
...
(p/'relaxed42-130-170-before.json').write_text(json.dumps(backup,indent=2,ensure_ascii=False)+'\n');(p/'relaxed42-130-170-decisions.json').write_text(json.dumps(audit,indent=2,ensure_ascii=False)+'\n')
