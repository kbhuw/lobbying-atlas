...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round53-root-draft.json'))
rows=json.load(open(r/'general-round53-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={39,55,60,68,93,33,46,48,50,81,90,92,1,2,3,4,5,6,8,11,12,13,14,16,17,20,21,22,26,27,28,31,32,34,35,36,43,45,49,51,58,65,66,67,70,71,72,73,74,75,76,77,78,79,80,84,85,86,91,94,95,96,97,98,99}
...
assert len(p)==100
(r/'general-round53-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
