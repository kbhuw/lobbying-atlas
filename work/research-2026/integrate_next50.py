...
r=pathlib.Path('work/research-2026');site=pathlib.Path('lobbying-map');p=json.load(open(r/'reviewed.json'));batch=json.load(open(r/'combined-next50.json'));holds=json.load(open(site/'research/registry-match-holds.json'));assert len(batch)==50
p.update(batch)
assert len(p)==539
...
if not any(b['name']=='further-50-root-checked' for b in m['batches']):m['batches'].append({'name':'further-50-root-checked','ids':sorted(batch)})
(site/'research/reviewed-2026-manifest.json').write_text(json.dumps(m,indent=2)+'\n')
f=site/'scripts/test-reviewed-2026.mjs';f.write_text(f.read_text().replace('length,489','length,539').replace('489 exact IDs','539 exact IDs'))
