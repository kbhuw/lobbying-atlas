...
p=Path('work/research-2026'); master=p/'reviewed.json';d=json.loads(master.read_text());c=json.loads((p/'relaxed-address-candidates42.json').read_text());a=json.loads((p/'relaxed-address42-agent-210-270.json').read_text())
accept={211,215,237,242,252,253,255,256,265}; backup={};audit=[];now=datetime.datetime.now(datetime.timezone.utc).isoformat()
for r in a:
 i=r['original_index']
...
(p/'relaxed42-210-270-before.json').write_text(json.dumps(backup,indent=2,ensure_ascii=False)+'\n');(p/'relaxed42-210-270-decisions.json').write_text(json.dumps(audit,indent=2,ensure_ascii=False)+'\n')
