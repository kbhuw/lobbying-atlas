...
p=Path('work/research-2026');f=p/'reviewed.json';d=json.loads(f.read_text());a=json.loads((p/'legal-pages45-held-reviewed.json').read_text());cs={r['id']:r for r in json.loads((p/'relaxed-address-candidates42.json').read_text())};before={};audit=[];now=datetime.datetime.now(datetime.timezone.utc).isoformat()
for r in a:
 id=r['id'];
 if r['decision']!='confirm':continue
 rec={'id':id,'decision':'hold'}
 if d[id]['review_outcome']=='confirmed':rec['decision']='already_confirmed';audit.append(rec);continue
 if id in ['5aae96511d725772']:
  rec['reason']='Unbounded Inc/including match has no exact legal name evidence.' if id=='12ee893e54bc5fb3' else 'Legal policy says Corporation but contact footer says LLC; conversion/history needs clarification.';audit.append(rec);continue
 cache=json.loads((p/'legal-pages45'/f'{id}.json').read_text());url=r['official_url'];q=r['exact_legal_name_quote']
