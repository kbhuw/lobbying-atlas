...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round34-c-input.json'));exec((r/'check_general34a.py').read_text().split('p=json.load(open(root/')[0]);dest=r/'round34c-image-audit';dest.mkdir(exist_ok=True)
def fetch(iv):
 ...
 try:
  ...
  if 'svg' in mime:out['svg']=True;return out
