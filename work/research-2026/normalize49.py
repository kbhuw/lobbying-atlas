...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round49-input.json'));e={};p={}
for part in ['first33','second33','third34']:e.update(json.load(open(r/f'round49-{part}-evidence.json')))
for part in ['first33','second33','third34','audit','extra','ownership-extra','root']:
 f=r/f'round49-{part}-corrections.json'
 if f.exists():
...
for i,row in enumerate(rows):
 ...
 add(i,name,desc,v.get('website',''),kind=('Organization' if v.get('kind') in ['Private company','Public company','Nonprofit / tax-exempt'] else v.get('kind','Organization')),own=own,sources=src,notes=notes,outcome=out,featured=i in [6,13,16,18,19,20,21,22,30,33,44,45,50,51,52,53,57,58,59,77,91,96])
(r/'general-round49-root-draft.json').write_text(json.dumps(p,indent=2)+'\n')
