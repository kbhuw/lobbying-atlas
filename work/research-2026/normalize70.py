...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round70-input.json'));e={};p={}
for part,start,end in [('first33',0,33),('second33',33,66),('third34',66,100)]:
 data=json.load(open(r/f'round70-{part}-evidence.json'))
...
for part in ['first33-audit','second33-audit','third34-audit','first33-extra','second33-extra','third34-extra','second33-final','third34-final','logo-root','root']:
 f=r/f'round70-{part}-corrections.json'
 if f.exists():
...
for i,row in enumerate(rows):
 ...
 add(i,name,desc,v.get('website') or '',kind=('Organization' if v.get('kind') in ['Private company','Public company','Nonprofit / tax-exempt'] else v.get('kind','Organization')),own=own,sources=src,notes=notes,outcome=out,featured=i in [1,5,6,8,9,10,12,15,17,19,20,24,27,28,29,30,31,34,36,37,43,44,45,46,47,50,56,57,58,60,61,65,67,72,75,76,79,80,81,82,86,87,88,89,92,93,95,96,97,98,99])
(r/'general-round70-root-draft.json').write_text(json.dumps(p,indent=2)+'\n')
