...
def one(item):
 ...
 try:
  ...
  for _,asset,kind in sorted(page.candidates)[:6]:
   ...
   if 'static.parastorage.com/client/pfavico.ico' in asset:continue
...
profiles=json.load(open(root/'public-eight.json'))
with concurrent.futures.ThreadPoolExecutor(max_workers=6) as ex:
 for id,r in ex.map(one,profiles.items()):print(id,r.get('http_status',r.get('error')),r.get('logo_kind','missing'),flush=True)
