...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round37-root-draft.json'))
rows=json.load(open(r/'general-round37-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={3, 4, 6, 7, 9, 12, 13, 14, 15, 16, 17, 20, 21, 24, 27, 29, 30, 34, 35, 36, 37, 40, 41, 43, 44, 45, 46, 48, 49, 50, 51, 56, 57, 60, 61, 63, 66, 68, 71, 74, 76, 78, 79, 80, 81, 83, 85, 89, 90, 91, 93, 94, 96, 97, 98}
...
for k,v in p.items():
 ...
 if k==rows[6]['id'] and c.get('logo_http_status')==200:
  v.update(logo_url=c['logo_url'],logo_kind='logo',logo_source_url=c['logo_source_url'],logo_status='official_site_asset')
...
assert len(p)==100
(r/'general-round37-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
