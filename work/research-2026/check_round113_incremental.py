...
def one(item):
 ...
 try:
  final,mime,b=get(u);out.update(final_url=final,http_status=200);page=Page(p['name'],final);page.feed(b.decode('utf-8','replace'));out['text']=' '.join(page.text)[:16000]
  for raw in page.ld:
...
p=json.load(open(root/"round113-root-incremental-evidence.json"))
def needs(k,v):
