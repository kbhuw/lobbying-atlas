...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round68-root-draft.json'))
rows=json.load(open(r/'general-round68-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={12,32,92,39,44,46,49,54,55,58,59,60,61,62,63,95,2,6,7,8,9,10,13,14,15,17,18,19,20,21,23,24,25,27,28,29,31,33,34,37,38,41,43,45,47,48,51,52,56,66,67,68,69,71,72,74,75,78,79,81,82,83,84,85,86,87,88,89,90,91,94,97,98,99}
...
assert len(p)==100
(r/'general-round68-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
