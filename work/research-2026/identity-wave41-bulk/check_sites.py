...
previous={x['id'] for batch in ['identity-wave30-bulk','identity-wave31-bulk','identity-wave32-bulk','identity-wave33-bulk','identity-wave34-bulk','identity-wave35-bulk','identity-wave36-bulk','identity-wave37-bulk','identity-wave38-bulk','identity-wave39-bulk','identity-wave40-bulk'] for x in json.load(open(ROOT/'work/research-2026'/batch/'queue.json'))}
rows=[x for x in rows if x['id'] not in previous]
rows.sort(key=lambda x:p[x['id']]['name'].casefold());rows=rows[:500]
  ...
  for j in range(3):
   ...
   try:
    ...
    for e in x['evidence']:
     ...
     if len(addr)>=12 and len(city)>=3 and re.search(r'(?<![A-Z0-9])'+re.escape(addr)+r'(?![A-Z0-9])',nt) and re.search(r'(?<![A-Z0-9])'+re.escape(city)+r'(?![A-Z0-9])',nt):
