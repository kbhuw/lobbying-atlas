...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round73-input.json'));exec((r/'check_general73_final.py').read_text().split('p=json.load(open(root/')[0]);dest=r/'round73-image-audit';dest.mkdir(exist_ok=True)
def fetch(iv):
 ...
 return out
results=list(concurrent.futures.ThreadPoolExecutor(max_workers=8).map(fetch,[(i, rows[i]) for i in [31,44,92]]))
