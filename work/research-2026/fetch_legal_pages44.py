...
root=pathlib.Path(__file__).resolve().parents[2];b=root/'work/research-2026/legal-pages44';b.mkdir(exist_ok=True);p=json.load(open(root/'work/research-2026/reviewed.json'));rows=json.load(open(root/'work/research-2026/relaxed-address-candidates42.json'));rows=[x for x in rows if p[x['id']]['review_outcome']!='confirmed' and re.search(r'\b(inc|llc|corp|corporation|holdings?|lp|limited)\b',x['matches'][0]['registration']['client_name'],re.I)][:150];(b/'queue.json').write_text(json.dumps(rows,indent=2))
