...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round75-input.json'));exec((r/'check_round75_incremental.py').read_text().split('p=json.load(open(root/')[0]);dest=r/'round75-image-audit-extra';dest.mkdir(exist_ok=True)
def fetch(iv):
 ...
 return out
results=list(concurrent.futures.ThreadPoolExecutor(max_workers=8).map(fetch,[(i,rows[i]) for i in [10,11,12,13,14,15,16,19,20,21,22,24,25,27,33,35,36,38,39]]))
