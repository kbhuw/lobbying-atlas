...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round48-input.json'));e={};p={}
for part in ['first33','second33','third34']:e.update(json.load(open(r/f'round48-{part}-evidence.json')))
for part in ['first33','second33','third34','audit','extra','ownership-extra','root']:
 f=r/f'round48-{part}-corrections.json'
 if f.exists():
...
for i,row in enumerate(rows):
 ...
 add(i,name,desc,v.get('website',''),kind=('Organization' if v.get('kind') in ['Private company','Public company','Nonprofit / tax-exempt'] else v.get('kind','Organization')),own=own,sources=src,notes=notes,outcome=out,featured=i in [4,6,8,9,11,12,13,14,15,16,23,24,27,28,29,33,40,41,42,43,45,48,52,53,54,55,62,65,66,68,72,73,79,85,88,89,97,98])
(r/'general-round48-root-draft.json').write_text(json.dumps(p,indent=2)+'\n')
