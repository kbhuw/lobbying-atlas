...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round72-input.json'));e={};p={}
for part,start,end in [('first33',0,33),('second33',33,66),('third34',66,100)]:
 data=json.load(open(r/f'round72-{part}-evidence.json'))
...
for part in ['first33-audit','second33-audit','third34-audit','first33-extra','second33-extra','third34-extra','second33-final','third34-final','logo-root','root']:
 f=r/f'round72-{part}-corrections.json'
 if f.exists():
...
for i,row in enumerate(rows):
 ...
 add(i,name,desc,v.get('website') or '',kind=('Organization' if v.get('kind') in ['Private company','Public company','Nonprofit / tax-exempt'] else v.get('kind','Organization')),own=own,sources=src,notes=notes,outcome=out,featured=i in [0,1,7,8,9,11,18,19,22,23,24,30,36,39,40,41,43,44,45,46,47,48,49,50,51,64,65,70,71,73,77,80,81,82,88,91,92,93,94,98,99])
(r/'general-round72-root-draft.json').write_text(json.dumps(p,indent=2)+'\n')
