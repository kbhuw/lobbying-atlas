...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round38-root-draft.json'))
rows=json.load(open(r/'general-round38-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={0,2,4,6,10,11,13,16,17,18,19,20,21,24,27,29,30,31,32,35,36,37,38,39,40,41,43,44,45,47,50,51,53,54,56,57,58,60,61,63,64,65,66,67,68,71,72,73,74,75,76,77,79,80,81,82,83,85,89,91,92,95,98,99}
...
assert len(p)==100
(r/'general-round38-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
