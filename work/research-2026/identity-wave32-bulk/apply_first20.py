...
R=pathlib.Path(__file__).resolve().parent;ROOT=R.parents[2];master=ROOT/'work/research-2026/reviewed.json';p=json.load(open(master));shutil.copyfile(master,R/'first20-before.json');q=json.load(open(R/'matches.json'))[:20];audit=[]
for item in q:
 i=item['id'];x=p[i];m=item['matches'][0];e=m['registration']
 if i in ['af2e6d2421a5d742','6b1c59a604436324','96d884a883507f54','1160dac5e5add057','5aca58282d7f4885','32475299753e9812']:
 ...
 if i=='b8f7c5ebd30b3d5d':x['notes']='Official footer identifies Cadence Solutions, Inc. at the filed address. This healthcare company is distinct from Cadence Design Systems. Ownership remains unverified.'
 if i=='eaf50f96a75db523':x['notes']='The official footer explicitly identifies By Light Professional IT Services LLC at the filed address. Do not merge historical Inc. labels without separate continuity evidence.'
 if i=='55c4f2bd74694cb3':x['name']='Breaking Barriers to Substance Use Recovery';x['notes']='The website identifies a coalition. A separate incorporated entity or tax status is not inferred.'
 if i=='c22d10cf2154b502':x['notes']='Official company story describes Born Primitive as veteran owned and operated. Ownership percentages and legal structure remain unverified.'
