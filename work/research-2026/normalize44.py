...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round44-input.json'));e={};p={}
for part in ['first33','second33','third34']:e.update(json.load(open(r/f'round44-{part}-evidence.json')))
for part in ['first33','second33','third34','audit','root']:
 f=r/f'round44-{part}-corrections.json'
 if f.exists():
...
for i,row in enumerate(rows):
 ...
 add(i,name,desc,v.get('website',''),kind=('Organization' if v.get('kind') in ['Private company','Public company','Nonprofit / tax-exempt'] else v.get('kind','Organization')),own=own,sources=src,notes=notes,outcome=out,featured=i in [1,4,5,8,13,14,16,17,22,23,33,34,36,39,41,45,48,52,59,66,78,79,80,87,89,90,91,92,94,95,98,99])
(r/'general-round44-root-draft.json').write_text(json.dumps(p,indent=2)+'\n')
