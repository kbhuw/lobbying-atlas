...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round57-root-draft.json'))
rows=json.load(open(r/'general-round57-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={76,80,81,84,85,86,87,90,92,93,3,5,17,33,35,37,52,54,55,56,58,61,0,1,2,4,7,8,9,10,11,12,13,14,16,18,21,23,24,25,26,27,28,29,30,36,38,39,40,41,42,43,44,45,46,48,50,51,53,59,60,62,63,64,65,68,69,70,71,72,73,74,77,78,79,82,83,88,89,91,95,96,97,98,99}
...
assert len(p)==100
(r/'general-round57-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
