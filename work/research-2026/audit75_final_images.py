...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round75-input.json'));exec((r/'check_round75_incremental.py').read_text().split('p=json.load(open(root/')[0]);dest=r/'round75-image-audit-final';dest.mkdir(exist_ok=True)
def fetch(iv):
 ...
 return out
results=list(concurrent.futures.ThreadPoolExecutor(max_workers=8).map(fetch,[(i,rows[i]) for i in [26,48,54,57,58,64,65,66,67,68,69,70,71,72,73,74,75,76,77,78,79,80,82,84,85,86,87,88,89,90,91,92,93,94,96,97,98,99]]))
