...
p={}
for label in ['a','b','c']:
 for x in json.load(open(root/'college-scorecard'/f'round2-{label}-input.json')):
  school=x['location_matches'][0]['school'];u=school['INSTURL'].strip();u=u if u.startswith(('https://','http://')) else 'https://'+u;p[x['id']]={'name':school['INSTNM'],'website':u}
