...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round76-input.json'));exec((r/'check_round76_incremental.py').read_text().split('p=json.load(open(root/')[0]);dest=r/'round76-image-audit-lastfive';dest.mkdir(exist_ok=True)
def fetch(iv):
 ...
 return out
results=list(concurrent.futures.ThreadPoolExecutor(max_workers=8).map(fetch,[(i,rows[i]) for i in [95,96,97,98,99]]))
