...
r=pathlib.Path('work/research-2026');site=pathlib.Path('lobbying-map');p=json.load(open(r/'reviewed.json'));batch=json.load(open(r/'college-scorecard/review90-rootchecked.json'));holds=json.load(open(site/'research/registry-match-holds.json'));assert len(batch)==90
p.update(batch)
assert len(p)==984
...
if not any(b['name']=='college-scorecard-first90' for b in m['batches']):m['batches'].append({'name':'college-scorecard-first90','ids':sorted(batch)})
(site/'research/reviewed-2026-manifest.json').write_text(json.dumps(m,indent=2)+'\n')
f=site/'scripts/test-reviewed-2026.mjs';f.write_text(f.read_text().replace('length,894','length,984').replace('894 exact IDs','984 exact IDs'))
