...
items=[]
for n in range(2):
 for x in json.load(open(root/f'tax-large-{n}.json')):
  raw=x['tax_evidence']['filed_website'].strip().lower();url=raw if raw.startswith(('https://','http://')) else 'https://'+raw
  items.append((x['id'],{'name':x['name'],'website':url}))
