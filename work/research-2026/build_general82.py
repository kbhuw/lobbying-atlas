...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round82-root-draft.json'))
rows=json.load(open(r/'general-round82-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={2, 5, 6, 7, 8, 11, 12, 13, 15, 16, 17, 19, 21, 23, 25, 31, 32, 33, 34, 35, 36, 37, 38, 41, 42, 47, 49, 50, 51, 52, 53, 56, 60, 64, 65, 67, 68, 69, 70, 73, 74, 76, 77, 80, 84, 86, 88, 89, 95}
...
assert len(p)==100
(r/'general-round82-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
