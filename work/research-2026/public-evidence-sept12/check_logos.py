...
b=pathlib.Path('work/research-2026/public-evidence-sept12');c=b/'logos';c.mkdir(exist_ok=True);d=json.load(open('work/research-2026/reviewed.json'));q=[]
for r in json.loads((b/'website-results.json').read_text()):
 ...
 z=r['logo_candidates'][1 if r['name']=='Trimble Inc' else 0];q.append({'id':r['id'],'name':r['name'],'source':r['resolved_url'],'url':urllib.parse.urljoin(r['resolved_url'],z['src']),'alt':z.get('alt','')})
def run(r):
 try:
...
sheet.save(c/'contact-sheet.png');print('Downloaded',len(ok),'of',len(rr))
