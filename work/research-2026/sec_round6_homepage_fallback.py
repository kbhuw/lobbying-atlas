...
p={}
for f in root.glob('sec-round6-*-researched.json'):
 a=json.load(open(f));a={v['id']:v for v in a} if isinstance(a,list) else a
 for k,v in a.items():
  old=cache/(k+'.json')
  if old.exists() and json.load(open(old)).get('error') and v.get('website'):
   u=urllib.parse.urlparse(v['website']);base=urllib.parse.urlunparse((u.scheme,u.netloc,'/','','',''))
   if base!=v['website']:p[k]={'name':v['name'],'website':base}
with concurrent.futures.ThreadPoolExecutor(max_workers=8) as ex:
 for k,v in ex.map(one,p.items()):print(k,v.get('http_status',v.get('error')),v.get('final_url'),flush=True)
