...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round34a-root-draft.json'))
rows=json.load(open(r/'general-round34-a-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
hold={rows[i]['id'] for i in [0,1,6,9,11,12]}
for k,v in p.items():
...
assert len(p)==30
(r/'general-round34a-30-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
