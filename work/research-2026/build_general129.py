...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round129-root-draft.json'))
rows=json.load(open(r/'general-round129-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow=set(json.load(open(r/'round129-logo-review.json'))['approved_indices'])
reviewed_assets={x['id']:x.get('url','') for x in json.load(open(r/'round129-image-audit-final/fetches.json'))}
...
assert len(p)==100
(r/'general-round129-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
