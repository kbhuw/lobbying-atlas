...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round53-input.json'));e={};p={}
for part in ['first33','second33','third34']:e.update(json.load(open(r/f'round53-{part}-evidence.json')))
for part in ['first33','second33','third34','audit','extra','ownership-extra','ffp','tax-us','root']:
 f=r/f'round53-{part}-corrections.json'
 if f.exists():
...
for i,row in enumerate(rows):
 ...
 add(i,name,desc,v.get('website',''),kind=('Organization' if v.get('kind') in ['Private company','Public company','Nonprofit / tax-exempt'] else v.get('kind','Organization')),own=own,sources=src,notes=notes,outcome=out,featured=i in [0,1,2,19,21,32,34,35,40,47,48,49,53,54,66,67,68,73,75,76,77,78,79,80,81,82,83,84,85,86,87,88,90,91,92,93,96,97,98,99])
(r/'general-round53-root-draft.json').write_text(json.dumps(p,indent=2)+'\n')
