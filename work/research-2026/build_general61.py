...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round61-root-draft.json'))
rows=json.load(open(r/'general-round61-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow=set([43,0, 1, 3, 6, 7, 8, 9, 10, 11, 13, 14, 15, 16, 17, 18, 19, 20, 23, 24, 25, 26, 30, 35, 36, 38, 41, 42, 44, 45, 46, 48, 49, 51, 54, 55, 56, 58, 59, 60, 61, 63, 64, 66, 67, 68, 69, 71, 72, 73, 75, 76, 77, 82, 83, 84, 86, 94, 96, 97, 99])
...
assert len(p)==100
(r/'general-round61-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
