...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round50-root-draft.json'))
rows=json.load(open(r/'general-round50-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={1,2,3,4,5,6,7,10,14,16,18,19,20,22,23,24,25,27,30,33,34,37,38,39,41,42,48,49,52,53,54,56,57,59,60,61,62,63,64,65,66,67,68,69,70,71,72,73,75,76,78,82,83,84,85,86,87,88,89,91,92,93,94,95,96,99}
...
assert len(p)==100
(r/'general-round50-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
