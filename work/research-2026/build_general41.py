...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round41-root-draft.json'))
rows=json.load(open(r/'general-round41-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={1,3,4,5,6,7,9,10,11,12,13,15,16,18,20,21,23,25,26,27,28,29,31,32,33,34,35,38,39,42,43,45,46,47,48,50,51,52,53,54,55,59,63,67,68,69,70,71,72,73,74,75,76,77,79,81,82,83,84,85,87,88,89,90,91,92,93,94,95,97,98}
...
assert len(p)==100
(r/'general-round41-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
