...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round72-root-draft.json'))
rows=json.load(open(r/'general-round72-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={8,12,13,35,38,52,56,66,72,81,85,96,0,1,6,7,9,10,14,15,16,17,18,19,22,23,24,27,28,30,31,32,39,40,41,43,44,45,46,47,48,50,51,54,55,57,60,64,65,67,70,71,73,75,76,77,80,88,91,92,94,97,98,99}
...
assert len(p)==100
(r/'general-round72-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
