...
r=pathlib.Path('work/research-2026');rows=json.load(open(r/'general-round75-input.json'));exec((r/'check_round75_incremental.py').read_text().split('p=json.load(open(root/')[0]);dest=r/'round75-image-audit-more';dest.mkdir(exist_ok=True)
def fetch(iv):
 ...
 return out
results=list(concurrent.futures.ThreadPoolExecutor(max_workers=8).map(fetch,[(i,rows[i]) for i in [30,40,41,42,43,44,45,46,47,49,50,51,53,55,56,59,60,61,62,63]]))
