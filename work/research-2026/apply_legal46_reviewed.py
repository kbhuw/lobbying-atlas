...
p=Path('work/research-2026');f=p/'reviewed.json';d=json.loads(f.read_text());a=json.loads((p/'legal-pages46-reviewed.json').read_text());cs={r['id']:r for r in json.loads((p/'relaxed-address-candidates42.json').read_text())};before={};audit=[];now=datetime.datetime.now(datetime.timezone.utc).isoformat()
for r in a:
 id=r['id'];
 ...
 cache=json.loads((p/'legal-pages46'/f'{id}.json').read_text());url=r['official_url'];q=r['exact_legal_name_quote']
