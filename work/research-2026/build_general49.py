...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round49-root-draft.json'))
rows=json.load(open(r/'general-round49-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={0,2,3,4,7,8,9,11,14,16,17,22,25,28,30,31,32,35,36,37,38,40,41,43,44,45,46,47,49,50,51,52,53,54,55,56,59,63,64,66,67,69,70,71,72,73,77,78,80,82,86,88,89,91,93,94,96,97,98}
...
assert len(p)==100
(r/'general-round49-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
