...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round56-root-draft.json'))
rows=json.load(open(r/'general-round56-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={1,2,3,5,15,20,21,24,39,49,69,75,78,82,86,71,95,96,0,4,7,8,9,10,11,12,13,14,16,17,18,19,22,23,25,26,28,29,31,34,37,40,43,44,45,48,52,58,59,60,61,63,65,66,67,70,72,73,74,76,77,79,81,83,84,85,89,90,91,94,99}
...
assert len(p)==100
(r/'general-round56-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
