...
r=pathlib.Path('work/research-2026');e={};rows=json.load(open(r/'general-round145-input.json'))
for label,start,end in [('a',0,34),('b',34,67),('c',67,100)]:
 p=json.load(open(r/f'round145-{label}-evidence.json'));assert set(p)=={x['id'] for x in rows[start:end]};e.update(p)
for k,v in json.load(open(r/'round145-root-patches.json')).items():
...
assert len(e)==100
(r/'round145-root-incremental-evidence.json').write_text(json.dumps(e,indent=2)+'\n')
