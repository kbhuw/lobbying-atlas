...
r=pathlib.Path('work/research-2026');p=json.load(open(r/'general-round51-root-draft.json'))
rows=json.load(open(r/'general-round51-input.json'))
# Hold parent assets where the exact subsidiary branding has not been established.
allow={0, 4, 6, 7, 8, 9, 16, 17, 18, 19, 20, 23, 27, 28, 29, 30, 31, 35, 38, 39, 41, 42, 44, 45, 46, 47, 49, 50, 51, 54, 55, 56, 60, 61, 63, 64, 65, 66, 68, 69, 70, 71, 72, 73, 74, 75, 78, 80, 81, 84, 86, 87, 91, 92, 95}
allow.update({10,11,12,24,25,57,82,83,85,90,93,94,96,97,98,99})
...
assert len(p)==100
(r/'general-round51-100-rootchecked.json').write_text(json.dumps(p,indent=2)+'\n')
