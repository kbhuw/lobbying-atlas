...
r=pathlib.Path('work/research-2026');e={};rows=json.load(open(r/'general-round95-input.json'))
for label,start,end in [('a',0,34),('b',34,67),('c',67,100)]:
 p=json.load(open(r/f'round95-{label}-evidence.json'));assert set(p)=={x['id'] for x in rows[start:end]};e.update(p)
for k,v in json.load(open(r/'round95-root-patches.json')).items():
...
for v in e.values():
 ...
 if not v['website'] and all('lda.gov/' in s['url'] or 'disclosurespreview.house.gov/' in s['url'] for s in v['sources']):
  v['review_outcome']='unresolved';v['description']='Lobbying client reporting '+v['description'].rstrip('.')+'. Exact current organization identity has not been independently verified.'
assert len(e)==100
(r/'round95-root-incremental-evidence.json').write_text(json.dumps(e,indent=2)+'\n')
